...
export default function ChatbotScreen() {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 bg-[#0F1D17]">
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View className="flex-1 p-4">
          ...
          {/* ✅ Render Chatbot or Goal Tracking Based on Active Tab */}
          {activeTab === "Chat" ? (
            <View className="flex-1 mb-4">
              <ChatMessages messages={messages} />
              {showSuggestions && <SuggestedQuestions onSelect={handleSendMessage} />}
              {loading && <Text className="text-white text-center mt-2">AI is thinking...</Text>}
